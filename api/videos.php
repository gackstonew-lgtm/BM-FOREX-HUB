<?php
// Admin/Public API Endpoint: api/videos.php
...
switch ($method) {
  case 'GET':
    ...
    if (!empty($search)) {
      ...
      $data = array_values(array_filter($data, function($v) use ($needle) {
        return strpos(strtolower($v['title'] ?? ''), $needle) !== false;
      }));
    }
  ...
  case 'PATCH':
    ...
    $payload = [
      ...
      'video_url'   => $input['video_url'] ?? $input['url'] ?? '',
    ...
    if (empty($payload['title']) || empty($payload['video_url'])) {
